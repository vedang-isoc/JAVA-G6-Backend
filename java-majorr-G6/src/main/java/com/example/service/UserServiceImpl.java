...
public class UserServiceImpl implements UserService{
	...
	public String createUser(User user) {
		...
		User u=new User(user.getUserId(), user.getUsername(), user.getEmail(), user.getPassword(), false, false, "user");
		p.setUser(u);
		u.setProfile(p);
		ur.save(u);
	...
	@Override
	public User forLogin(String userName, String email, String password) {
		User u = new User();
		if(userName == null) {
			u.setEmail(ur.findByEmail(email));
		}else {
			u.setUsername(ur.findByUserrname(userName));
		}
		u.setPassword(ur.findByPassword(password));
		return u;
	}

	@Override
	public Optional<User> getuserDetails(int id) {
		return ur.findUserDetails(id);
	}

	@Override
	public String getPassword(int id, String password) {
		String oldPassword =  ur.findPassword(id);
	...
	@Override
	public String getEmailSendTempPass(String email) {
		if(ur.findByEmail(email) == null) {
			return "wrong email address";
		}else {
			return "correct email but aftewards send temporary password generated by system and also send email to user";
		}
	...
	public Profile createProfile(int userid, Profile profile) {
		...
		User u = user.get();
		ur.save(u);
		Profile p = pfr.findByUser(user.get());
		int id = p.getProfileId();
		// use stream API to convert each starting to capital
		profile.setProfileId(id);
		profile.setUser(user.get());
		return pfr.save(profile);
	}

	@Override
	public Profile getProfileDetails(int userid) {
		return pfr.findProfile(userid);
	}

	@Override
	public Profile editProfile(int userid, Profile profile) {
		Optional<User> user = ur.findUserDetails(userid);
		profile.setUser(user.get());
		return pfr.save(profile);
	}
